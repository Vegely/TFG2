...
int main(void)
{
    ...
    if (!sk || !pk || !ct || !ss) {
        ...
        // Keygen
        if (hqc_keypair(pk, sk) != 0) printf("Keypair generation failed\r\n");
        else printf("Keypair generation successful\r\n");

        // Encaps
        if (hqc_encapsulate(ct, ss, pk) != 0) printf("Encapsulation failed\r\n");
        else printf("Encapsulation successful\r\n");
        
        // Decaps
        if (hqc_decapsulate(ss, ct, sk) != 0) printf("Decapsulation failed\r\n");

...
int main(void)
{
    ...
    if (hqc_init(HQC_SECURITY_256) != 0) { /* Error */ }

    // Allocate Memory
    size_t sz_pk = hqc_get_public_key_size();
    size_t sz_sk = hqc_get_secret_key_size();
    size_t sz_ct = hqc_get_ciphertext_size();
    size_t sz_ss = hqc_get_shared_secret_size();

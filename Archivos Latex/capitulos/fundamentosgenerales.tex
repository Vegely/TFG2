...
Se elabora esta sección siguiendo el artículo \cite{kyber-spec-2021}. CRYSTALS-Kyber es un algoritmo postcuántico basado en anillos algebraicos sobre polinomios, denotados como \(R_q\):
\begin{equation}
	R_q \coloneqq \dfrac{\mathbb{Z}_q[X]}{X^n + 1}
\end{equation}

Según la especificación \cite{kyber-spec-2021}, la implementación de referencia utiliza $q=3329$ y $n=256$. Esta elección de parámetros es crítica, pues habilita el uso de la Transformada Teórica de Números (\acrshort{ntt}) para la multiplicación polinómica. El uso de la \acrshort{ntt} reduce la complejidad temporal de la operación desde $\mathcal{O}(n^2)$, correspondiente al método clásico de multiplicación, a $\mathcal{O}(n \log n)$. Para un análisis detallado del véase \cite{cryptoeprint:2024/585}.
\newline

Para la correcta definición de la \acrshort{ntt}, es requisito indispensable que \(n| (q-1)\), es decir, que \(n\) divida a \((q-1)\).  Esta condición asegura la existencia de raíces enésimas de la unidad en el cuerpo finito $\mathbb{Z}_q$ (dado que $q$ es primo), consecuencia directa del teorema 2.8 de \cite{lidl_finite_1997}: 
\newline

\textbf{Proposición 1}
	Para \(n\), \(q>1\), el cuerpo  \(\mathbb{Z}_q\) tiene una raíz enésima de la unidad si y solo sí \(n| (q-1)\)
\newline
...
\newline
\subsubsection{Transformada Teórica de Números o Number Theoretic Transform (NTT)}
...
\subsubsection{\acrshort{lwe} en anillos}
...
En cuanto a la eficiencia en la reducción del tamaño de las claves, esta puede observarse claramente a través del ejemplo de la tabla \ref{tab:LWE-RLWE}.
\begin{table}[H]
	\centering
...
A continuación, se presenta la tabla \ref{tab:KyberParams} con los parámetros del esquema Kyber1024 empleado en este trabajo fin de grado.
\begin{table}[h]
	\centering
	\renewcommand{\arraystretch}{1.2}
...
Para elaborar esta sección se parte del artículo de Saber adjuntado con la proposición al NIST \cite{saber-spec-2020}. En Saber al igual que en Kyber se trabaja en un anillo de la forma:
\begin{equation}
...
A continuación, se presenta la tabla \ref{tab:SaberParams} con los parámetros del esquema FireSaber empleado en este trabajo fin de grado.
\begin{table}[H]
	\centering
	\renewcommand{\arraystretch}{1.2}
...
A continuación, se presenta la tabla \ref{tab:HQCParams} con los parámetros del esquema \acrshort{hqc}-5 empleado en este trabajo fin de grado.
\begin{table}[H]
	\centering
	\renewcommand{\arraystretch}{1.2}
	\begin{tabular}{lcccccccccc}
...
\begin{figure}[H]
	...
	\end{circuitikz}
	\caption{Representación de las equivalencias de seguridad entre los distintos esquemas de seguridad posible. Si existe una flecha entre dos esquemas \(A\) y \(B\) implica que si se da \(A\) entonces \(B\) también se cumple. Los números denotan los teoremas del artículo \cite{CCA2} que demuestran estas relaciones.}
...
\begin{itemize}
	...
	\item \textbf{Ataque de plantilla}: El esquema Kyber presenta actualmente dos vectores de ataque debido a su funcionamiento: el muestreo binomial \cite{Peng_Kuhn_2025} y la operación de la \acrshort{ntt} \cite{cryptoeprint:2019/795}. Este riesgo podría mitigarse mediante algoritmos de enmascarado. 
	\newline

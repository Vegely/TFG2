...
Por tanto, el polinomio \(X^{256} + 1\) se puede factorizar sobre el cuerpo \(\mathbb{Z}_q\). En la implementación concreta del esquema Kyber, este polinomio se descompone en 128 factores cuadráticos. Como resultado, la Transformada (NTT) de un polinomio en el anillo \(R_q\) se representa como un vector de 128 polinomios de grado 1.
\newline
...
Aplicando la NTT a un polinomio \(f\in R_q\) se obtiene:
\begin{equation}
...
\begin{figure}[H]
	\centering
		...
		\end{circuitikz}
	\label{fig:NTTFigure}
	\caption{Representación del cálculo de un producto de polinomios mediante NTT en comparación con los algoritmos clásicos.}
\end{figure}
\newpage

Tras este breve inciso sobre la aplicación concreta de la NTT en el algoritmo Kyber, utilizado en el presente trabajo, se procede a analizar los fundamentos teóricos de la Transformada Teórica Numérica según el artículo \cite{cryptoeprint:2024/585}.
\subsubsection{LWE}
\cite{LWElearning}
